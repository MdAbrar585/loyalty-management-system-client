...
import { createProject, loadProject } from "../../redux/actions/projectAction";
import {
  CLEAR_ERRORS,
  CREATE_PROJECT_RESET,
} from "../../redux/constants/projectConstant";

const AddProject = ({ setState, setPages, pages }) => {
  ...
  const onSubmit = (data) => {
    ...
    dispatch(createProject(token.access_token, myForm));
  };
  ...
  const {
    ...
    createdProjectData,
  } = useSelector((state) => state.createProjects);

  useEffect(() => {
    if (createdProjectError) {
      alert.error(createdProjectError.message);
    ...
    if (createdProjectSuccess) {
      ...
      console.log("success", createdProjectSuccess);
