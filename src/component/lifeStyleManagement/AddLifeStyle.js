...
import { CREATE_LIFE_STYLE_RESET } from "../../redux/constants/lifeStyleConstant";
import {
  createLifeStyleFunc,
  loadLifeStyleFunc,
...
const AddLifeStyle = ({ setState, pages }) => {
  // var today = new Date();
  ...
  const onSubmit = (data) => {
    ...
    dispatch(createLifeStyleFunc(token.access_token, myForm));
  };

  const {
    createdLifeStyleError,
    createdLifeStyleSuccess,
  } = useSelector((state) => state.createLifeStyle);
  ...
  useEffect(() => {
    ...
    if (createdLifeStyleError) {
      alert.error(createdLifeStyleError.message);
      // setState({ right: false });
      dispatch({ type: CLEAR_ERRORS });
    }

    if (createdLifeStyleSuccess) {
      alert.success("New Life Style Created Successfully!");
      setState({ right: false });
      console.log("success", createdLifeStyleSuccess);
      dispatch({ type: CREATE_LIFE_STYLE_RESET });
