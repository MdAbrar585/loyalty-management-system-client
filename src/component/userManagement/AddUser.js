...
const AddUser = ({ setState, setPages, pages }) => {
  ...
  const onSubmit = (data) => {
    ...
    dispatch(createUser(token.access_token, myForm));
  };
  ...
  const {
    ...
    createdUserError,
    createdUser,
  } = useSelector((state) => state.createUsers);
  ...
  useEffect(() => {
    if (createdUserError) {
      alert.error(createdUserError.message);
      setBtnLoader(false);
